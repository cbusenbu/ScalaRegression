...
class StochasticGradientDescent(trainData:NormalizedData,
  ...
  def wlim:DenseVector[Double] = {
    ...
    val train:DenseMatrix[Double] = movePredictColToEnd(preTrain)
    val coefficients = DenseVector.fill(train.cols){0.0}
  ...
  def setPredictColumn(columnNum:Int):Unit ={
    ...
    wLimVector = DenseVector.fill(1){0.0}
  }
